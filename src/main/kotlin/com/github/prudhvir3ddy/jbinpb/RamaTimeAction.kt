...
class RamaTimeAction : AnAction(), DumbAware {

    override fun getActionUpdateThread() = ActionUpdateThread.BGT

    override fun actionPerformed(e: AnActionEvent) {
        RamaDialogWrapper(e.project).show()
    }

    @Suppress("MagicNumber")
    private class RamaDialogWrapper(project: Project?) : DialogWrapper(project) {
        ...
        init {
            init()
            title = "Rama's Divine Progress Demo"
        }
        ...
        private fun createPanel(indeterminate: Boolean, modeless: Boolean): JComponent {
            val text = if (indeterminate) "Indeterminate (Rama blessing continuously)" else "Determinate (Rama's divine journey)"
            ...
            val victory = JBLabel("🏆 Jai Shree Ram!")
